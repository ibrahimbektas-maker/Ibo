...
POSITION_MATCH_TOLERANCE = 1.0    # pts de tolerance pour matcher la position par prix
...
def find_position_id(direction, entry_price):
    ...
    for p in positions:
        ...
        if abs(open_level - entry_price) <= POSITION_MATCH_TOLERANCE:
            ...
            if deal_id:
                candidates.append((deal_id, open_level))
    if not candidates:
        return None
    # Si plusieurs matchs (hedging), on prend le plus proche
    candidates.sort(key=lambda c: abs(c[1] - entry_price))
